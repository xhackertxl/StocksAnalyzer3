...
public class StockListConfigurationService {
	...
	private StockListConfigurationService() {
		...
		resourcesPaths[0] = "all_list.properties";
	...
	private Properties loadProperties(String... resourcesPaths) {
		...
		for (String location : resourcesPaths) {
			...
			try {
				...
				Log.v("Properties not found from path:{}, {} ", location + ex.getMessage());
			} catch (Exception ex) {
				Log.v("Could not load properties from path:{}, {} ", location  + ex.getMessage());
				ex.printStackTrace();
			} finally {
				if (is != null) {
					try {
